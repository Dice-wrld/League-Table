...
export default function SettingsPage({
  ...
  const Section = ({ id, title, children }: { id: string; title: string; children: React.ReactNode }) => (
    <div className="bg-slate-800/60 rounded-xl border border-slate-700 overflow-hidden">
      <button onClick={() => setOpenSection(openSection === id ? '' : id)} className="w-full flex items-center justify-between px-4 py-3.5 text-left">
        <span className="text-white font-semibold text-sm">{title}</span>
        <span className="text-slate-400 text-xs">{openSection === id ? '▲' : '▼'}</span>
      </button>
      {openSection === id && <div className="px-4 pb-4 border-t border-slate-700/50 pt-4">{children}</div>}
    </div>
  );
    ...
    <div className="space-y-3">
      ...
      <Section id="tournament" title="🏆 Tournament">
        <div className="space-y-3">
          <div>
            ...
            <input type="text" value={tournament.tournamentName} onChange={(e) => onUpdateName(e.target.value)}
            ...
            <div className="flex gap-2">
              {(['league', 'queue'] as TournamentMode[]).map((m) => (
                <button key={m} onClick={() => onSetMode(m)}
                  className={cn('flex-1 py-2 rounded-lg text-sm font-medium transition-colors border', tournament.mode === m ? 'bg-purple-600 border-purple-400 text-white' : 'bg-transparent border-slate-600 text-slate-400 hover:text-white')}>
          ...
          <div>
            <label className="text-slate-400 text-xs mb-1 block">Tournament Length (matches)</label>
            <div className="flex gap-2">
              <input type="number" min="1" value={pendingLength} onChange={(e) => setPendingLength(e.target.value)} placeholder="Unlimited"
              ...
              <button onClick={() => { const n = parseInt(pendingLength); onSetLength(isNaN(n) || n <= 0 ? null : n); showToast('✅ Saved'); }}
                className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg transition-colors text-sm font-medium">Save</button>
            </div>
            {tournament.tournamentLength && (
              <button onClick={() => { onSetLength(null); setPendingLength(''); showToast('Removed length limit'); }}
                className="text-xs text-red-400 hover:text-red-300 mt-2 underline decoration-dotted">Remove limit</button>
            )}
          </div>
          <div className="flex gap-2 pt-1">
            <button onClick={onAddTournament} className="flex-1 py-2 bg-green-700 hover:bg-green-600 text-white rounded-lg text-sm font-medium transition-colors">+ New</button>
            {collection.tournaments.length > 1 && (
              <select value={collection.currentId} onChange={(e) => onSelectTournament(parseInt(e.target.value))} className="flex-1 bg-slate-700 border border-slate-600 text-white rounded-lg px-2 py-2 text-sm focus:outline-none">
      ...
      <Section id="teams" title="👥 Teams">
        <div className="space-y-2">
          {tournament.teams.map((team) => (
            <div key={team.id}>
              <div className="flex items-center gap-2">
                ...
                <input type="text" value={teamNames[team.id] ?? team.name}
                ...
                <button onClick={() => setEditingTeam(editingTeam === team.id ? null : team.id)}
                  className="px-2 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-xs transition-colors">🎨</button>
                <button onClick={() => { if (tournament.teams.length <= 2) { showToast('Need at least 2 teams'); return; } if (confirm('Remove?')) onRemoveTeam(team.id); }}
                  className="px-2 py-2 bg-red-800 hover:bg-red-700 text-white rounded-lg text-xs transition-colors">✕</button>
              </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {PRESET_COLORS.map((c) => (
                      <button key={c} onClick={() => onUpdateAppearance(team.id, c, team.emoji)}
                        className={cn('w-7 h-7 rounded-full border-2 transition-transform hover:scale-110', team.color === c ? 'border-white scale-110' : 'border-transparent')}
                        style={{ background: c }} />
                    ))}
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {PRESET_EMOJIS.map((e) => (
                      <button key={e} onClick={() => onUpdateAppearance(team.id, team.color, e)}
                        className={cn('w-8 h-8 rounded-lg text-lg flex items-center justify-center transition-colors', team.emoji === e ? 'bg-purple-600' : 'bg-slate-700 hover:bg-slate-600')}>
                        {e}
                      </button>
                    ))}
          ...
          <button onClick={onAddTeam} className="w-full py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm transition-colors mt-1">+ Add Team</button>
        </div>
      </Section>

      {/* Data */}
      <Section id="data" title="💾 Data & Export">
        <div className="space-y-3">
          <button onClick={onExport} className="w-full py-2.5 bg-orange-700 hover:bg-orange-600 text-white font-bold rounded-xl transition-colors text-sm">⬇️ Download JSON Backup</button>
          ...
          {showImport && (
            <div className="space-y-2">
              <textarea value={importText} onChange={(e) => setImportText(e.target.value)} placeholder="Paste tournament JSON…"
              ...
              <button onClick={() => { try { onImport(importText); setImportText(''); setShowImport(false); showToast('✅ Imported!'); } catch { showToast('❌ Invalid JSON'); } }}
                disabled={!importText.trim()} className="w-full py-2 bg-green-700 hover:bg-green-600 disabled:opacity-40 text-white font-bold rounded-xl transition-colors text-sm">Import</button>
            </div>
          )}
        </div>
      </Section>
      ...
      {/* Danger zone */}
      <Section id="danger" title="⚠️ Reset">
        <div className="space-y-2">
          <p className="text-slate-400 text-xs">Resets all match data and stats. Team names and settings are kept.</p>
          <button onClick={() => { if (confirm('Reset all match data?')) onResetTournament(); }}
            className="w-full py-2.5 bg-red-800 hover:bg-red-700 text-white font-bold rounded-xl transition-colors text-sm">🔄 Reset Tournament</button>
        </div>
      </Section>
      ...
      <div className="text-center py-4">
        <p className="text-purple-300/50 text-xs">
          ...
          <a href="https://github.com/Dice-wrld" target="_blank" rel="noopener noreferrer" className="text-purple-200/70 hover:text-white underline decoration-dotted transition-colors">Dice-wrld</a>
        </p>
